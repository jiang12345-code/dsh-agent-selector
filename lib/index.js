...
async function runHy3(ctx, task, cwd, timeoutMs, signal, modelId, memoryFile) {
  const py = firstExisting(PY_CANDIDATES)
  ...
  try {
    ...
    if (memoryFile) appendMemory(path.basename(memoryFile, '.md'), task, (parsed.text || ''))
...
async function dispatchAgent(ctx, args, exec) {
  ...
  if (r.target === 'hy3') {
    ...
    return runHy3(ctx, task, cwd, 600000, signal, r.cfg.hy3Model || 'hy3', memFile)
  }
...
export function apply(ctx) {
    ...
    req.on('end', async () => {
      ...
      try {
        ...
        if (method === 'mode.announce') {
          ...
          if (cfgAfterSet.delegateMode !== on) {
          ...
          const text = dmode >= 2
            ? '【全托管委派已开启】用户已在智能体选择器中选择 FULL 全托管（当前目标：' + combo + '）。此后用户发送的每一条消息——无论任务、讨论、追问、闲聊——请一律通过 agent_dispatch 工具（target="auto"）委派给所选智能体执行：你负责把消息打包为自包含 brief（委派通道的会话记忆会自动携带历史）、调用工具、然后向用户转述结果与出处头。没有任何豁免，禁止自己回答任何内容。委派失败时如实报告失败。'
            : dmode === 1
              ? '【委派模式已开启】用户已在智能体选择器中打开委派开关（当前目标：' + combo + '）。此后用户发送的任务类消息，请一律通过 agent_dispatch 工具（target="auto"）委派给所选智能体执行：你负责把任务补全为自包含 brief、调用工具、然后向用户转述结果与出处头。寒暄、提问、与你讨论方案的消息仍由你直接回答，不要委派。委派失败时如实报告失败，禁止自己代答冒充。'
  ...
  try {
    ctx.effect(() => ctx.on('session/event', (session, event) => {
      ...
      if (!dmode) return
      // v0.1.5c: observed log — evidence the guard actually saw a user message
      if (ctx.logger && typeof ctx.logger.info === "function") ctx.logger.info("agent-selector guard observed user message session=" + sessionId)
      else console.log("[agent-selector] guard observed user message session=" + sessionId)
      ...
      const text = dmode >= 2
        ? '【强约束 · 全托管委派】用户已开启全托管：刚刚的这条消息（无论任务、讨论、追问、闲聊）必须一律通过 agent_dispatch 工具（target="auto"）委派给 ' + combo + ' 执行。\n' +
          '【必做】① 立即调用 agent_dispatch，target="auto"，task=把用户这条消息原文+必要的上下文说明打包为自包含 brief（委派通道的会话记忆会自动携带历史，无需你复述全部历史，但当前消息的指代要补全）；② 原样转述工具返回值（含出处头）。\n' +
          '【禁做】禁止自己回答任何内容、禁止区分任务与闲聊、禁止省略 tool call。委派失败如实报告。'
        : '【强约束 · 委派模式守卫】用户刚发送了上面的任务类消息。你必须、且只能通过 agent_dispatch 工具（target="auto"）委派给 ' + combo + ' 执行。\n' +
          '【必做】① 立即调用 agent_dispatch，target="auto"，task=用户的原任务补全为自包含 brief；② 把工具返回值原样转述给用户（含出处头）。\n' +
          '【禁做】禁止自己回答、禁止省略 tool call、禁止编造结果。如实报告失败，不得代答冒充。'
      ...
      if (ctx.logger && typeof ctx.logger.info === "function") ctx.logger.info("agent-selector delegate GUARD FIRED session=" + sessionId + " combo=" + combo)
      else console.log("[agent-selector] GUARD FIRED session=" + sessionId + " combo=" + combo)
    }))
  } catch (e) {
    if (ctx.logger && typeof ctx.logger.error === "function") ctx.logger.error("agent-selector delegate guard SETUP FAILED: " + String(e && e.message || e))
    else console.error("[agent-selector] delegate guard SETUP FAILED: " + String(e && e.message || e))
  }
}
