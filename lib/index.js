...
import { acpDispatch } from './acp.js'
  ...
  if (r.target === 'hy3') {
    ...
    if (dmode >= 2) {
      ...
      try {
        ...
        // 会话级持久化 acpSessionId → 下一条消息走 session/load 续同一会话（原生多轮记忆）
        if (callSessionId && acp.acpSessionId) saveConfig(callSessionId, { acpSessionId: acp.acpSessionId })
        return clip('[hy3 · ACP 原生续聊 · ' + acp.sessionMode + ' · 耗时 ' + Math.round((acp.durationMs || 0) / 1000) + 's]\n\n' + (acp.text || ''))
      } catch (e) {
        const why = (e instanceof Error ? e.message : String(e)).slice(0, 300)
        console.error('[agent-selector] ACP 通道失败，回退 automation 桥: ' + why)
        const bridged = await runHy3(ctx, task, cwd, 600000, signal, r.cfg.hy3Model || 'hy3', memFile, dmode)
        return clip('[ACP 失败已回退: ' + why + ']\n\n' + bridged)
      }
    }
    return runHy3(ctx, task, cwd, 600000, signal, r.cfg.hy3Model || 'hy3', memFile, dmode)
  }
  ...
  try {
    ctx.effect(() => ctx.on('session/event', (session, event) => {
      ...
      const text = '【强约束 · 委派模式守卫】用户刚发送了上面的消息。你必须通过 agent_dispatch 工具（target="auto"）委派给 ' + combo + ' 执行' + tierNote + '。\n' +
        '【必做】① 立即调用 agent_dispatch，target="auto"，task=把用户这条消息打包为自包含 brief（会话记忆仅携带最近若干轮的截断摘要、非全文——涉及具体文件、函数、错误原文、代词所指对象的一律在 brief 里补全，不要依赖执行体记得住）；② 原样转述工具返回值（含出处头）。\n' +
