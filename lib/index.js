...
// ---- v0.3.0: 过程流环形缓冲（B 方案「WorkBuddy 过程流面板」）----
// taskId → { events, dropped }：onUpdate 事件按 taskId 累积，client 轮询 stream.pull 增量拉取。
// 绝对索引（dropped + events.length）保证滚动截断后 since 语义不错位；cap 200 条/任务，最多 20 个任务。
const STREAM_BUFFERS = new Map()
const STREAM_CAP = 200
const STREAM_MAX_TASKS = 20
function pushStreamEvent(taskId, evt) {
  if (!taskId || !evt) return
  let b = STREAM_BUFFERS.get(taskId)
  if (!b) {
    b = { events: [], dropped: 0 }
    STREAM_BUFFERS.set(taskId, b)
    if (STREAM_BUFFERS.size > STREAM_MAX_TASKS) STREAM_BUFFERS.delete(STREAM_BUFFERS.keys().next().value)
  }
  b.events.push(evt)
  if (b.events.length > STREAM_CAP) { b.events.splice(0, 50); b.dropped += 50 }
}
function pullStreamEvents(taskId, since) {
  const b = STREAM_BUFFERS.get(String(taskId || ''))
  if (!b) return { events: [], total: 0, dropped: 0 }
  const s = Math.max(0, (since | 0) - b.dropped)
  return { events: b.events.slice(s), total: b.dropped + b.events.length, dropped: b.dropped }
}
...
async function dispatchAgent(ctx, args, exec) {
  ...
  if (r.target === 'hy3') {
    ...
    const execHy3 = async (sig, taskId) => {
      ...
      if (dmode >= 2) {
        ...
        try {
          const acp = await acpDispatch({
            ...
            timeoutMs: 900000,
            // v0.3.0: 过程流外抛——思考/工具/输出 chunk 按 taskId 写环形缓冲（过程流面板数据源）
            // （v0.3.2：删除此处重复键的 v0.2.4 死代码——它引用未定义的 STREAM_BUF/LAST_TASK_ID，
            //  被本行覆盖虽不致命，但一旦被意外激活即 ReferenceError 打断 prompt 流）
            onUpdate: taskId ? (evt) => pushStreamEvent(taskId, evt) : undefined,
          })
    ...
    if (callSessionId) {
      ...
      // v0.3.0: 任务级过程流——taskId 贯穿 onUpdate 缓冲 / 受理文案 / 面板轮询
      const taskId = randomUUID()
      try { saveConfig(callSessionId, { lastTaskId: taskId }) } catch { /* 尽力 */ }
      pushStreamEvent(taskId, { kind: 'tool', text: '📡 委派已受理，等待 WorkBuddy 过程事件…', ts: Date.now() })
      const p = execHy3(undefined, taskId)
      p.then((text) => { pushStreamEvent(taskId, { kind: 'tool', text: '✅ 任务完成，结果已回注对话', ts: Date.now() }); fireFollowup(text) })
        .catch((e) => { pushStreamEvent(taskId, { kind: 'tool', text: '❌ 任务失败：' + (e instanceof Error ? e.message : String(e)).slice(0, 200), ts: Date.now() }); fireFollowup('[委派失败] ' + (e instanceof Error ? e.message : String(e))) })
        .catch(() => {}) // 链尾兜底：绝不让后台 promise 成为 unhandled rejection
      const combo = 'hy3 · ' + (r.cfg.hy3Model || 'hy3')
      return clip('✅ 已受理——' + combo + ' 后台执行中（任务ID ' + taskId.slice(0, 8) + '，预计数秒至数分钟，视任务复杂度；思考/工具/输出过程可在 设置 → 🤖 智能体选择器 → 📡 过程流 实时查看）。完成后结果将自动送达本对话，届时请转述给用户。')
    }
...
// 导出供门禁实测（stream.pull 的活 RPC 需 DSH 重启才挂载，模块级实测走这两个函数）
export { pushStreamEvent, pullStreamEvents }
...
export function apply(ctx) {
  ...
  const handler = async (req, res) => {
    ...
    req.on('end', async () => {
      ...
      try {
        if (method === 'agents.list') {          const probe = probeAgents(typeof args.sessionId === 'string' ? args.sessionId : '')
        ...
        if (method === 'stream.pull') {
          // v0.3.0 过程流面板：args={taskId, since(默认0)} → {events, total, dropped}
          // since 为绝对索引（含 dropped），返回增量事件与最新 total（=下次 since）
          return done({ ok: true, result: pullStreamEvents(args.taskId, args.since | 0) })
        }
  ...
  try {
    ctx.effect(() => ctx.on('session/event', (session, event) => {
      ...
      const text = '【强约束 · 全托管直通】用户刚发送了上面的消息。你必须通过 agent_dispatch 工具（target="auto"）委派给 ' + combo + ' 执行' + tierNote + '。\n' +
        '【必做】① 立即调用 agent_dispatch，target="auto"，task=用户消息**原话原样透传**（只允许在开头加元数据头：工作区/档位/记忆指针；**禁止改写、压缩、诊断、前置分析、补充说明——完整判断权归执行体**）；② agent_dispatch 会立即返回受理确认，把受理确认原样告知用户；委派结果送达后（以 dsh-agent-selector 来源的消息出现为准）**将结果原样呈现给用户**。\n' +
        '③【判断权移交】结果是否需要修正/继续/追问，由执行体在其输出中自行判断与说明——它的输出会以新消息形式回来，届时你只需继续原样呈现；主对话不做任何中间判断、不加总结评论、不充当发言人。\n' +
        '【禁做】禁止自己回答、禁止改写用户原话、禁止总结评论进度说明、禁止省略 tool call、禁止任何环节亲自修改。委派失败如实报告。'
      const message = {
        id: randomUUID(),
        role: 'user',
        content: [{ type: 'text', text }],
