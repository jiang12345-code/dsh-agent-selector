  ...
  if (r.target === 'hy3') {
    ...
    if (dmode >= 2) {
      ...
      try {
        ...
        const bridged = await runHy3(ctx, task, cwd, 600000, signal, r.cfg.hy3Model || 'hy3', memFile, dmode, r.cfg.effort || 'standard')
        return clip('[ACP 失败已回退: ' + why + ']\n\n' + bridged)
      }
    }
    return runHy3(ctx, task, cwd, 600000, signal, r.cfg.hy3Model || 'hy3', memFile, dmode, r.cfg.effort || 'standard')
  }
