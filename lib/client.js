...
window.__ModuleLoader__.load({ id: "dsh-agent-selector", factory: (require) => {
  ...
  function SelectorPanel() {
    ...
    var refresh = React.useCallback(function () {
      api("agents.list").then(function (j) {
  ...
  function ComposerAgentSelector() {
    var st = React.useState({ open: false, step: "agents", modelsFor: "", target: "auto", cfg: {}, avail: {} })
    var d = st[0], set = st[1]
    var rootRef = React.useRef(null)

    var load = React.useCallback(function () {
      api("config.get").then(function (j) {
    ...
    function save(cfgPatch) {
      api("config.set", { config: cfgPatch }).then(function (j) {
