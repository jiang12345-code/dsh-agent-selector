...
window.__ModuleLoader__.load({ id: "dsh-agent-selector", factory: (require) => {
  ...
  function ComposerAgentSelector(props) {
    var st = React.useState({ open: false, modeOpen: false, step: "agents", modelsFor: "", pendingModel: "", target: "auto", cfg: {}, avail: {}, modeToggling: false })
    var d = st[0], set = st[1]
    ...
    var load = React.useCallback(function () {
      api("config.get", { sessionId: sessionId }).then(function (j) {
        if (j && j.ok) set(function (p) { return { ...p, cfg: j.result || {}, target: (j.result && j.result.defaultTarget) || "auto", loadFail: "" } })
        else set(function (p) { return { ...p, loadFail: "config.get: " + ((j && j.error) || "fail") } })
      }).catch(function (e) { set(function (p) { return { ...p, loadFail: "连接失败: " + String(e) } }) })
      api("agents.list", { sessionId: sessionId }).then(function (j) {
    ...
    React.useEffect(function () {
      if (!d.open) return
      function onDoc(e) {
        try { if (rootRef.current && !rootRef.current.contains(e.target)) set(function (p) { return { ...p, open: false, modeOpen: false } }) } catch (err) {}
      }
    ...
    function pickModel(m) {
      ...
      if (!meta) return
      // v0.2.1: hy3 选中模型后追加第三步「推理档位」；其余目标保持两步直存
      if (meta.id === "hy3") { set(function (p) { return { ...p, step: "effort", pendingModel: m } }); return }
      var patch = { defaultTarget: meta.id }
      patch[meta.modelKey] = m
      save(patch)
    }
    // v0.2.1: 挡位语义值存 config.effort（off/standard/high，缺省 standard 由 host 侧兜底）
    function pickEffort(v) {
      save({ defaultTarget: "hy3", hy3Model: d.pendingModel, effort: v })
    }

    function toggleModeTo(next) {
      if ((next | 0) >= 2 && !window.confirm("即将开启档位 " + next + "（" + (next >= 3 ? "完全权限：委派可读写工作区内外文件" : "工作区内修改：委派可读写当前工作区文件") + "）。确认切换？")) return
    ...
    function comboLabel() {
      ...
      var eff = c.effort === "off" ? "🧠关" : c.effort === "high" ? "🧠高" : (c.effort === "standard" || !c.effort) ? "🧠标准" : ""
      if (t === "auto") return "跟随面板默认"
      if (t === "codex") return "Codex · " + (c.codexModel || "默认")
      if (t === "hy3") return "hy3 · " + (c.hy3Model || "hy3") + (eff ? " · " + eff : "")
    ...
    } else if (d.step === "effort") {
      // v0.2.1: 第三步——推理档位（仅 hy3 通道）。语义值与 host 侧 acpDispatch.effort 对齐。
      body.push([
        { id: "off", label: "🧠 关闭思考", desc: "model_is_thinking=0 / 不设思考档" },
        { id: "standard", label: "🧠 标准", desc: "默认推理档（中间档）" },
        { id: "high", label: "🧠 高", desc: "最高推理档（high/max）" },
      ].map(function (opt) {
        var active = (d.cfg && d.cfg.effort || "standard") === opt.id
        return React.createElement("div", {
          key: opt.id,
          onClick: function () { pickEffort(opt.id) },
          style: {
          ...
          React.createElement("span", { style: { width: 16, textAlign: "center", color: "#3fb950", fontSize: 12 } }, active ? "✓" : ""),
          React.createElement("span", { style: { flex: 1 } },
            React.createElement("div", { style: { fontSize: 13, color: "#e6edf3" } }, opt.label),
            React.createElement("div", { style: { fontSize: 11, color: "#8b949e" } }, opt.desc),
          ),
        )
      }))
      body.push(React.createElement("div", {
        key: "back", onClick: function () { set(function (p) { return { ...p, step: "models" } }) },
    ...
    return React.createElement("div", { ref: rootRef, style: { position: "relative", display: "inline-flex" } },
      ...
      React.createElement("button", {
        title: "委派权限档位：点选 OFF/仅查看/工作区内修改/完全权限",
        onClick: function () { set(function (p) { return { ...p, modeOpen: !p.modeOpen, open: false } }); load() },
        style: {
          background: "transparent", border: 0,
          color: d.modeOpen ? "#e6edf3" : "var(--dsh-fg-muted, #8b949e)",
          fontSize: 13, cursor: "pointer", padding: "3px 6px", borderRadius: 6,
          display: "inline-flex", alignItems: "center", gap: 4, whiteSpace: "nowrap",
          lineHeight: "20px",
        },
        onMouseEnter: function (e) { e.currentTarget.style.color = "#e6edf3" },
        onMouseLeave: function (e) { if (!d.modeOpen) e.currentTarget.style.color = "var(--dsh-fg-muted, #8b949e)" },
      },
        "⚡ " + (((d.cfg && d.cfg.delegateMode) | 0) === 1 ? "仅查看" : ((d.cfg && d.cfg.delegateMode) | 0) === 2 ? "工作区" : ((d.cfg && d.cfg.delegateMode) | 0) >= 3 ? "完全" : "关"),
        React.createElement("svg", {
          width: 12, height: 12, viewBox: "0 0 16 16", fill: "none",
          style: { opacity: 0.85, flexShrink: 0 },
        }, React.createElement("path", {
      ...
      d.modeOpen ? React.createElement("div", {
        style: {
          position: "absolute", bottom: "calc(100% + 8px)", right: 0, zIndex: 9999,
          minWidth: 270, background: "#161b22", border: "1px solid #30363d", borderRadius: 10,
          boxShadow: "0 8px 24px rgba(0,0,0,0.45)", padding: 6, fontFamily: "system-ui,-apple-system,sans-serif",
        },
      },
        React.createElement("div", { style: { fontSize: 11, color: "#8b949e", padding: "4px 8px 6px" } }, "⚡ 委派档位（点选即生效）"),
        [
          { mode: 0, title: "OFF · 关闭", desc: "常规行为：任务由当前对话直接处理" },
          { mode: 1, title: "仅查看", desc: "全委派但禁止修改项目文件——只读检索、分析、产出报告" },
          { mode: 2, title: "工作区内修改", desc: "全委派且允许读写当前工作区的文件与代码" },
          { mode: 3, title: "完全权限", desc: "全委派且等同 DSH 完全权限（工作区内外均可）" },
        ].map(function (opt) {
          var active = ((d.cfg && d.cfg.delegateMode) | 0) === opt.mode
          return React.createElement("div", {
            key: opt.mode,
            onClick: function () { if (!d.modeToggling && !active) { set(function (p) { return { ...p, modeToggling: true } }); toggleModeTo(opt.mode) } },
            style: {
              display: "flex", alignItems: "center", gap: 8, padding: "7px 8px", marginBottom: 2,
              borderRadius: 7, cursor: d.modeToggling ? "wait" : "pointer",
              background: active ? "#1c2128" : "transparent",
              border: active ? "1px solid " + (opt.mode >= 2 ? "#d29922" : "#238636") : "1px solid transparent",
            },
            onMouseEnter: function (e) { if (!active) e.currentTarget.style.background = "#1c2128" },
            onMouseLeave: function (e) { e.currentTarget.style.background = active ? "#1c2128" : "transparent" },
          },
            React.createElement("span", { style: { width: 16, textAlign: "center", color: opt.mode >= 2 ? "#d29922" : "#3fb950", fontSize: 12 } }, active ? "✓" : ""),
            React.createElement("span", { style: { flex: 1 } },
              React.createElement("div", { style: { fontSize: 13, color: "#e6edf3" } }, "⚡ " + opt.title),
              React.createElement("div", { style: { fontSize: 11, color: "#8b949e", marginTop: 1 } }, opt.desc),
            ),
          )
        }),
      ) : null,
      ...
      d.open ? React.createElement("div", {
        ...
        d.loadFail ? React.createElement("div", { style: { fontSize: 11, color: "#f85149", padding: "2px 8px 6px" } }, "⚠ " + d.loadFail + "——通道可用性未知，请勿盲选") : null,
        body,
      ) : null,
    )
  }
