...
/**
 * ACP 委派主入口。
 * @param {object} opts { task, cwd, acpSessionId?, poolKey?, dmode?, timeoutMs?, baseUrl?, knownHostToken? }
...
export async function acpDispatch(opts = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      return await runOnce(ep, reused, { task, cwd, wantedSessionId: sessionIdWanted, key, left, t0, hostToken, hostChanged })
    } catch (e) {
      lastErr = e
...
async function runOnce(ep, reused, o) {
  const { task, cwd, wantedSessionId, key, left, t0, hostToken, hostChanged } = o
  const base = ep.url
  let entry = reused
  let nextId = 1
  ...
  try {
    ...
    if (entry.acpSessionId) {
      ...
      if (lr && lr.__err) {
        ...
        entry.acpSessionId = ''
        sessionMode = 'new'
      } else if (lr && lr.result && typeof lr.result.sessionBlank === 'boolean') {
