...
/**
 * ACP 委派主入口。
 * @param {object} opts { task, cwd, acpSessionId?, poolKey?, dmode?, timeoutMs?, baseUrl?, knownHostToken?, effort? }
...
export async function acpDispatch(opts = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      return await runOnce(ep, reused, { task, cwd, wantedSessionId: sessionIdWanted, key, left, t0, hostToken, hostChanged, effort: String((opts && opts.effort) || '') })
    } catch (e) {
      lastErr = e
...
async function runOnce(ep, reused, o) {
  const { task, cwd, wantedSessionId, key, left, t0, hostToken, hostChanged, effort } = o
  const base = ep.url
  let entry = reused
  let nextId = 1
