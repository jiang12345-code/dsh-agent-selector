...
/**
 * ACP 委派主入口。
 * @param {object} opts { task, cwd, acpSessionId?, poolKey?, dmode?, timeoutMs?, baseUrl?, knownHostToken?, effort?, model? }
...
export async function acpDispatch(opts = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (attempt > 0 && left() < 30000) {
      throw lastErr instanceof Error ? lastErr : new Error(String(lastErr || 'ACP 重试预算耗尽'))
    }
    const reused = poolGet(key, ep.url, sessionIdWanted, hostToken)
    try {
      return await runOnce(ep, reused, { task, cwd, wantedSessionId: sessionIdWanted, key, left, t0, hostToken, hostChanged, effort: String((opts && opts.effort) || ''), model: String((opts && opts.model) || '') })
    } catch (e) {
      lastErr = e
...
async function runOnce(ep, reused, o) {
  const { task, cwd, wantedSessionId, key, left, t0, hostToken, hostChanged, effort, model } = o
  const base = ep.url
  let entry = reused
  let nextId = 1
  ...
  try {
    ...
    if (!entry.acpSessionId) {
      ...
      for (let i = 0; i < NEW_MAX_TRIES; i++) {
        ...
        entry.created = true
        const chk = await rpc('session/load', { sessionId: sid, cwd, mcpServers: [] }, Math.min(30000, left())).catch(() => null)
